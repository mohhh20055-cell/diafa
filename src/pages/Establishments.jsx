...
const Establishments = () => {
  ...
  const [search, setSearch] = useState(searchParams.get('ville') || '')
  ...
  useEffect(() => {
    ...
    if (ville) setSearch(ville)
    if (wil) setWilaya(wil)
    loadEstablishments({ ville, wilaya: wil, type })
  }, [searchParams])
  ...
  const handleSearch = () => {
    loadEstablishments({ ville: search, wilaya, type: typeFilter })
  }

  const filteredEstablishments = establishments.filter((est) =>
    est.nom?.toLowerCase().includes(search.toLowerCase()) ||
    est.ville?.toLowerCase().includes(search.toLowerCase()) ||
    est.wilaya?.toLowerCase().includes(search.toLowerCase())
  )
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <label className="block text-sm font-medium text-[#152A54] mb-2">{t('search')}</label>
              <input
                type="text"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
                placeholder={`${t('name')}, ${t('city')}...`}
                className="block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#CB9A56] focus:border-[#CB9A56]"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#152A54] mb-2">{t('wilaya')}</label>
              <select
                value={wilaya}
                onChange={(e) => {
                  setWilaya(e.target.value)
                  loadEstablishments({ ville: search, wilaya: e.target.value, type: typeFilter })
                }}
                className="block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#CB9A56] focus:border-[#CB9A56]"
              >
            ...
            <div>
              <label className="block text-sm font-medium text-[#152A54] mb-2">{t('type')}</label>
              <select
                value={typeFilter}
                onChange={(e) => {
                  setTypeFilter(e.target.value)
                  loadEstablishments({ ville: search, wilaya, type: e.target.value })
                }}
                className="block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#CB9A56] focus:border-[#CB9A56]"
              >
            ...
            <div className="flex items-end">
              <button
                ...
                className="w-full bg-[#152A54] text-white px-4 py-2 rounded-lg hover:bg-[#CB9A56] transition-colors font-medium"
              >
                {t('filter')}
              </button>
            </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-gray-400 mt-2">{t('tryChangingSearch')}</p>
          </div>
        ) : (
          <>
            <p className="text-sm text-slate-500 mb-6">
              {filteredEstablishments.length} {t('establishmentsFound')}
            </p>
                ...
                return (
                  <Link key={establishment.id} to={`/etablissements/${establishment.id}`}>
                    <HoverRevealCard
                      ...
                      badge={establishment.type === 'hotel' ? t('hotel') : establishment.type === 'mraqed' ? t('mraqed') : t('maison')}
                      ...
                      price={minPrice != null && isFinite(minPrice) && minPrice > 0 ? `${Math.round(minPrice).toLocaleString('fr-FR')} DA` : t('onDemand')}
                      priceUnit={t('perNight')}
                      onBook={() => {}}
                    />
                  </Link>
                )
