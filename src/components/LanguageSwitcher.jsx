  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        ...
        <svg className={`w-3 h-3 text-[#CB9A56] transition-transform ${open ? 'rotate-180' : ''}`} viewBox="0 0 24 24" fill="none">
          <path d="M6 9l6 6 6-6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </button>

      {open && (
        <div className="absolute end-0 top-full mt-2 min-w-[140px] rounded-xl border border-[#CB9A56]/20 bg-white shadow-lg overflow-hidden z-50">
          {LANGUAGES.map((lang) => (
            <button
              key={lang.code}
              ...
              className={`flex w-full items-center justify-between px-4 py-2 text-sm transition ${
                lang.code === current.code
                  ? 'bg-[#CB9A56]/10 text-[#152A54] font-bold'
                  : 'text-slate-600 hover:bg-[#152A54]/5'
              }`}
            >
